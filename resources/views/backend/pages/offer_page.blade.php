...
<div class="container-fluid flex-grow-1 container-p-y pt-5">
    <div class="row">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form method="POST" action="{{route('offer.info.store')}}" enctype="multipart/form-data">
                    @csrf
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="report-table" class="table mb-0">
                            ...
                            <tbody>
                                @foreach ($offer_info as $info)
                                    <tr>
                                        <td><img src="{{asset('uploads/pages/offer')}}/{{$info->image}}" alt class="img-fluid" style="width: 150px"></td>
                                        <td>{{$info->button}}</td>
                                        <td>{{$info->url}}</td>
                                        <td>{{$info->created_at->format('d M Y')}}</td>
                                        <td>
                                            <a href="{{route('offer.info.edit', $info->id)}}" class="btn btn-info btn-sm"><i class="feather icon-edit"></i>&nbsp;Edit </a>
                                            <a href="{{route('offer.info.delete', $info->id)}}" class="btn btn-danger btn-sm"><i class="feather icon-trash-2"></i>&nbsp;Delete </a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
